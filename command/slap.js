...
export default {
  ...
  author: "حمودي سان",
  role: "member",
  description: "دمج صورة الشخص على قالب Slap",
  ...
  async execute({ api, event, args }) {
    ...
    try {
      const imagePath = await generateSlap(userId);
      api.sendMessage({
        attachment: fs.createReadStream(imagePath)
      }, event.threadID);
    } catch (error) {
      console.error("Error while running slap command:", error);
      api.sendMessage("حدث خطأ أثناء تنفيذ الأمر ❌", event.threadID);
    }
  }
};

async function generateSlap(userId) {
  ...
  const template = await jimp.read("https://i.imgur.com/Mh9qvX7.png"); // مثال قالب صفعة
  ...
  const outputPath = "slap_result.jpg";
