...
export default {
  ...
  author: "حمودي سان",
  role: "member",
  description: "دمج صورة الشخص على قالب Kiss",
  ...
  async execute({ api, event, args }) {
    ...
    try {
      const imagePath = await generateKiss(userId);
      api.sendMessage({
        attachment: fs.createReadStream(imagePath)
      }, event.threadID);
    } catch (error) {
      console.error("Error while running kiss command:", error);
      api.sendMessage("حدث خطأ أثناء تنفيذ الأمر ❌", event.threadID);
    }
  }
};

async function generateKiss(userId) {
  ...
  const template = await jimp.read("https://i.imgur.com/fp1JcSy.png"); // مثال قالب قبلة
  ...
  const outputPath = "kiss_result.jpg";
